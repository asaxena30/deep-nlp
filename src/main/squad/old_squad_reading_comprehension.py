...
qa_module = QAModuleWithAttentionModified(embedding, device = device)
...
if not skip_model_training:
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        for batch in train_dataloader:
            ...
            # scheduler.step()
            optimizer.step()
            print(start_index_loss, end_index_loss, total_loss)
            iteration_count += 1
...
with SummaryWriter(comment='QAModule') as summaryWriter:
    summaryWriter.add_graph(qa_module, (sample_question_batch_input, sample_passage_batch_input), True)
