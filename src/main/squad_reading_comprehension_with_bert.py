...
skip_tuning_bert_model: bool = True
enable_pdb_checkpoints: bool = False
...
def collate_with_padding(batch):
    if not isinstance(batch[0], SquadTensorInstanceForBert):
...
def get_squad_dataset_from_file(file_path: str) -> SquadDatasetForBert:
    ...
    for squad_qa_instance_as_dict in instances:
        ...
        instance_tensor = SquadTensorInstanceForBert(torch.tensor(all_token_ids, dtype = torch.long, device = device),
    ...
    for data_item in train_dataloader:
        ...
        if use_checkpointing:
            ...
            bert_model_output: Tuple[torch.Tensor, torch.Tensor] = bert_qa_module(input_ids = data_item[0],
                                                                                  ...
                                                                                  skip_tuning_bert_model = skip_tuning_bert_model)
